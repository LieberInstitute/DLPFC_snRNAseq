...
all_run <- c("Br2720_mid", "Br2720_post", "Br2743_ant", "Br2743_mid", "Br3942_ant",
...
runs_R3.1 <- list.files(path = "~/CCC_snRNA_archive/CCC_snRNA_R3.1/",
                        recursive = TRUE) |>
...
runs_R3.0 <- list.files(path = "~/CCC_snRNA_archive/CCC_snRNA_R3.0/",
                        recursive = TRUE) |>
    unglue_data(patterns = "{Sample}/{file_name}") |>
    dplyr::filter(Sample!="log")
